...
namespace GlocalCart.API.Controllers
{
    ...
    public class OrdersController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateOrder([FromBody] CreateOrderDto dto) =>
            Ok(await _orderService.CreateOrderAsync(UserId, dto));

        [HttpGet]
        public async Task<IActionResult> GetMyOrders([FromQuery] int page = 1, [FromQuery] int pageSize = 20) =>
            Ok(await _orderService.GetBuyerOrdersAsync(UserId, page, pageSize));

        [HttpGet("{id}")]
        public async Task<IActionResult> GetOrder(int id) => Ok(await _orderService.GetOrderByIdAsync(UserId, id));
        ...
        public async Task<IActionResult> GetOrderLogs(int id) =>
            Ok(await _orderService.GetOrderLogsAsync(UserId, id));

        // === SELLER ===
        [HttpGet("seller")]
        [Authorize(Roles = "Seller,Admin")]
        public async Task<IActionResult> GetSellerOrders([FromQuery] int page = 1, [FromQuery] int pageSize = 20) =>
            Ok(await _orderService.GetSellerOrdersAsync(UserId, page, pageSize));
        ...
        public async Task<IActionResult> CreateShipment(int id, [FromBody] CreateShipmentDto dto) =>
            Ok(await _orderService.CreateShipmentAsync(UserId, id, dto));

        [HttpGet("{id}/shipment")]
        public async Task<IActionResult> GetShipment(int id) =>
            Ok(await _orderService.GetShipmentAsync(UserId, id));
    }
}
