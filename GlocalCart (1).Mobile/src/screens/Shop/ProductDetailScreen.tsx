...
export default function ProductDetailScreen() {
  ...
  const shopId = Number(product?.sellerId || 0);
  ...
  const handleChat = () => {
    ...
    if (!shopId) {
      Alert.alert('Khong the mo chat', 'San pham nay chua co thong tin nguoi ban.');
      return;
    }
  ...
  const handleToggleFollowShop = async () => {
    ...
    if (!shopId) {
      Alert.alert('Khong the theo doi', 'San pham nay chua co thong tin nguoi ban.');
      return;
    }
